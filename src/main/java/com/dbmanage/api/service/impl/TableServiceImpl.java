...
public class TableServiceImpl implements  TableService {
    ...
    @Autowired
    public TableServiceImpl(AppProperties appProperties) {
    ...
    public void validateField(ValidateFieldRequest request) throws ValidationException {
        if (request == null || request.getField() == null) {
            throw new ValidationException(appProperties.getErrorMessages().getValidation().getFieldEmpty());
        }
        ...
        if (!StringUtils.hasText(field.getName())) {
            throw new ValidationException(appProperties.getErrorMessages().getValidation().getFieldNameEmpty(), "name");
        }

        // 验证字段名格式
        if (!field.getName().matches("^[a-zA-Z_][a-zA-Z0-9_]*$")) {
            throw new ValidationException(appProperties.getErrorMessages().getValidation().getFieldNameFormat(), "name");
        }

        // 验证字段类型
        if (!StringUtils.hasText(field.getType())) {
            throw new ValidationException(appProperties.getErrorMessages().getValidation().getFieldTypeEmpty(), "type");
        }
        ...
        if (!supportedTypes.contains(field.getType().toLowerCase())) {
            throw new ValidationException(
                MessageResolver.format(
                    appProperties.getErrorMessages().getValidation().getFieldTypeNotSupported(), 
                    field.getType()
                ), 
                "type"
            );
    ...
    public void createTable(CreateTableRequest request) throws SQLException, ValidationException {
        ...
        try (Connection connection = connectionService.getConnection(request.getConnectionId())) {
            ...
            if (StringUtils.hasText(request.getDatabaseName())) {
                // 根据数据库类型使用不同方式设置当前数据库
                String dbType = databaseTypeService.getNormalizedDatabaseType(request.getDatabaseType());
                switch (dbType) {
                    case "mysql":
                        try (PreparedStatement stmt = connection.prepareStatement("USE " + escapeTableName(request.getDatabaseName(), dbType))) {
                            stmt.execute();
                        }
                        break;
                    case "postgresql":
                        connection.setSchema(request.getDatabaseName());
                        break;
                    case "sqlserver":
                        try (PreparedStatement stmt = connection.prepareStatement("USE " + escapeTableName(request.getDatabaseName(), dbType))) {
                            stmt.execute();
                        ...
                        break;
                    case "oracle":
                        // Oracle不需要切换schema，已经在连接字符串中指定
                        break;
                    default:
                        logger.warn("未知数据库类型 {}，无法切换数据库，将使用默认数据库", dbType);
                }
                
                logger.info("已切换到数据库: {}", request.getDatabaseName());
            } else {
                logger.warn("未指定数据库名称，将使用默认数据库");
            }
